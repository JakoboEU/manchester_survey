...
species_count = function(species_group_column) { 
  ...
  data %>% select(title, species) %>% separate_rows(species, convert = TRUE, sep = ",") %>% na.omit() %>% group_by(species) %>% mutate(species = trimws(species)) %>% summarise(cell_count = n()) %>% arrange(cell_count)  
}
```
